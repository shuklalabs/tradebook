...
Open your current running app → Configuration → Data & Backup → Export Backup
Save that `.json` file somewhere you'll find it
Then run this updated version (`npm start` or the new installer)
Once it opens, go to Configuration → Data & Backup → Import Backup and
...
You need Node.js installed (the LTS version). Then, in this
...
One-time setup
Create a GitHub account if you don't have one (free) at github.com.
Create a new repository for this app — it can be private, that's fine,
auto-update works the same either way. Name it anything, e.g. `aks-trade-journal`.
Open `package.json` in this project and find this section near the bottom:
```json
   "publish": {
     "provider": "github",
...
Create a GitHub Personal Access Token (this lets your computer upload
...
Set that token as an environment variable before publishing. In PowerShell:
```
...
Bump the version number in `package.json`, e.g. `"version": "1.0.1"`.
Run:
```
   npm run publish
   ```
...
That's it. Anyone already running an older version of the app will be offered
